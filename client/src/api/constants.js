const BASE_URL = 'https://dustind.net/api/v1/closet';

const HEROKU_BASE_URL = 'https://mighty-chamber-19968.herokuapp.com/virtual/api/v1.0/upload';

// Basic CRUD functinoality

export const GET_CLOTHING_ITEMS = 'http://localhost:8080/virtual/api/v1.0/closet'; //BASE_URL;
export const ADD_CLOTHING_ITEM = 'https://mighty-chamber-19968.herokuapp.com/virtual/api/v1.0/confirmation'; // BASE_URL.concat('/add');
export const ADD_CLOTHING_ITEM_IMAGE = 'http://localhost:8080/virtual/api/v1.0/closet/add_image';

export const UPLOAD_ITEM_IMAGE = 'https://mighty-chamber-19968.herokuapp.com/virtual/api/v1.0/upload'; //'http://localhost:8080/virtual/api/v1.0/upload';//
